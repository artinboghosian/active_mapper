...
describe ActiveMapper::Adapter::Arel do
  ...
  describe '#where' do
    it 'finds the matching records' do
      ids = adapter.where(User) { |u| u.age < 35 }.all.map { |record| record['id'] }
    ...
    it 'offsets the matching records' do
      ids = adapter.where(User, offset: 1).all.map { |record| record['id'] }
